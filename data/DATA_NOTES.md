...
- **Cinemática de la pared — clave para la SDF dependiente del tiempo de SLGNN:**
  ...
  - ⚠️ **La fórmula del PDF es internamente inconsistente:** tomada literalmente, $4-4t$ se hace negativa para $t>1$ (llegando a $-2$ en $t=1.5$), lo que contradice al propio PDF cuando afirma que "la dirección de rotación permanece constante" y que la desaceleración es "simétrica". La lectura consistente —implementada en `slgnn.sdf.dynamical_cylinder_omega`— es el perfil triangular $\omega(t) = 2\pi\max(0, \min(4t,\,4-4t))$: acelera 0→2 en $[0,0.5]$, desacelera simétricamente 2→0 en $[0.5,1.0]$, reposo después. **Pendiente verificar contra los datos** (observar cuándo cesa el arrastre de pared en CASE08) al llegar a la fase de extrapolación.
