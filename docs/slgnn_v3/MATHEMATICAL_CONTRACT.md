...
| `Psi` tangencial | implementada y opcional | `test_tangential_channel_opposes_slip_and_is_passive`, `test_tangential_force_produces_spin_through_JT` |
| `Psi` rotacional directa | desactivada | `DissipationHead` falla si se activa |
| Perfil `v3-H` | reservado | `build_router` falla con los tres requisitos nombrados |

Consecuencia medible y probada: **`Delta L` es exactamente cero** en las
configuraciones históricas del MVP sin `Psi_tau`
(`test_symmetry.py::test_mvp_produces_exactly_zero_angular_momentum`). Con
`v3_c_tangential.yaml`, `Delta L` es no nulo, conserva momento angular total y
recibe gradiente desde la pérdida rotacional.
