...
| Tests estructurales en `float64` | implementado | 123 tests en `tests/v3` |
...
| **`Psi` tangencial y rotacional** | **contrato** | `DissipationHead` falla si se activan |
...
### `Delta L` es exactamente cero

Sin canal tangencial no hay torque. El modelo predice `Delta L = 0` con
igualdad exacta, y hay un test que lo afirma
...
| `scripts/v3/_common.py`, `run_all_benchmarks.py` | Utilidades de CLI; no cambian ningún contrato. |
...
1. fricción impulsiva, cono de Coulomb y spin oblicuo;
2. memoria tangencial `M`, sticking/sliding y lifecycle completo;
3. router híbrido `v3-H`;
4. pared móvil y extrapolación al cilindro rotatorio;
5. rollouts largos (50, 100);
6. cierre `C`, solo si quedan residuales reproducibles.
