"""Genera `docs/slgnn_v3/RESULTS_MVP.md` desde los artefactos JSON de las corridas.
...
`_NARRATIVE` y citan los números que el propio script inserta.
    ...
    python scripts/slgnn_v3/make_results_report.py
"""
...
RESULTS = REPO_ROOT / "results/slgnn_v3"
...
def build(out_path: Path) -> None:
    ...
    A("")
    A("Generado por `scripts/slgnn_v3/make_results_report.py` desde los JSON de las")
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=REPO_ROOT / "docs/slgnn_v3/RESULTS_MVP.md")
    args = ap.parse_args()
    build(args.out)
    return 0
