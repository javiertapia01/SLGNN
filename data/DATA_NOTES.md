...
- **Cinemática de la pared — clave para la SDF dependiente del tiempo de SLGNN:**
  ...
  - ✅ **RESUELTO contra los datos: vale la fórmula literal del PDF, no la lectura triangular.** El PDF *parece* internamente inconsistente porque $4-4t$ se hace negativa para $t>1$ (llegando a $-2$ en $t=1.5$), contradiciendo su propia afirmación de que "la dirección de rotación permanece constante". Ante la duda, `slgnn.sdf.dynamical_cylinder_omega` adoptó provisionalmente el perfil triangular $\omega(t) = 2\pi\max(0, \min(4t,\,4-4t))$. **Los datos lo refutan.** Dos señales independientes medidas sobre CASE08 (ver `src/twin/cylinder.py` y el paso 0 de `experiments/exp_H_hysteresis.py`):
    1. **Signo de la rotación del lecho.** La velocidad angular media de las partículas próximas a la pared lateral es positiva hasta $t\approx1.0$ s y claramente **negativa** entre 1.0 y 1.5 s ($\approx -2.3, -3.8, -2.9$ rad/s). El perfil triangular predice reposo en ese tramo: nada arrastraría la carga en sentido inverso.
    2. **Spin propio de las partículas.** $|\omega_i\cdot\hat z|$ tiene **dos** máximos, en $t\approx0.5$ s y $t\approx1.5$ s ($\approx 49$ y $45$ rad/s), que son exactamente los dos extremos de $|\omega(t)|$ de la fórmula literal. El triangular predice un único máximo en 0.5 s y nada después de 1.0 s.

    Conclusión: **el tambor invierte el sentido de giro**; lo equivocado es la prosa del PDF, no su fórmula. Trayectoria angular total: una revolución completa hacia adelante hasta $t=1.0$ s, media revolución hacia atrás hasta $t=1.5$ s.

    ⚠️ **Consecuencia para SLGNN:** `slgnn.sdf.dynamical_cylinder_omega` entrega una velocidad de pared incorrecta para $t>1$ s. Eso afecta directamente a $\mathbf v_W(\mathbf x,t)$ —el canal que la v2 introdujo precisamente para este caso— y por tanto a cualquier rollout de extrapolación (E2-b). El perfil verificado está implementado en `twin.cylinder.omega_pdf_literal` y anclado por `tests/test_twin_loop.py`; queda pendiente decidir si se corrige en `src/slgnn/`.
...
- [x] Verificar `omega(t)` del cilindro contra los datos: vale la **fórmula literal** del PDF (el tambor invierte el giro), no la lectura triangular. Ver §5.
- [x] Verificar el eje de gravedad del cilindro: es **−y** (la carga sedimenta en $y\approx-0.027$ m, con el eje del tambor en $z$), igual que en los datasets de caja. El tambor es por tanto un tambor rodante genuino: gravedad perpendicular al eje de rotación.
