...
Produce `results/v3_mvp/audit_temporal/{audit.json, audit_summary.csv}` y
`docs/v3/DATA_AUDIT.md`.

    python scripts/v3/audit_temporal.py --config configs/experiments/audit_temporal.yaml
...
def write_markdown(path: Path, report: dict) -> None:
    ...
    A("")
    A("Generado por `scripts/v3/audit_temporal.py`. **No editar a mano**: vuelve a")
    ...
    A("")
    A("El informe previo `Informe_Estrategia_Entrenamiento_SLGNN.md` asumía un")
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=REPO_ROOT / "results/v3_mvp/audit_temporal")
    ap.add_argument("--doc", type=Path, default=REPO_ROOT / "docs/v3/DATA_AUDIT.md")
    args = ap.parse_args()
