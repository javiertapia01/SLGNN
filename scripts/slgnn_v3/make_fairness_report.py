"""Genera `docs/slgnn_v3/AUDITORIA_EQUIDAD.md` desde los JSON de las corridas.
...
COMPARE = REPO_ROOT / "results/slgnn_v3/compare"
...
def build(out: Path) -> None:
    ...
    A("")
    A("Generado por `scripts/slgnn_v3/make_fairness_report.py`. **No editar a mano.**")
    A("")
    A(_NARRATIVE_INTRO)
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path,
                    default=REPO_ROOT / "docs/slgnn_v3/AUDITORIA_EQUIDAD.md")
    args = ap.parse_args()
    build(args.out)
    return 0
