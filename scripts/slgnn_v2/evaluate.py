...
Uso:
    python scripts/slgnn_v2/evaluate.py \
        --checkpoint checkpoints/slgnn_v2/gravity_rollout/best.pt \
        --config configs/slgnn_v2/gravity_rollout.yaml --case CASE06 --horizon 200

    # sobre el caso de extrapolación energética (held-out, nunca visto)
    python scripts/slgnn_v2/evaluate.py \
        --checkpoint checkpoints/slgnn_v2/gravity_rollout/best.pt \
        --config configs/slgnn_v2/gravity_rollout.yaml --case CASE07 \
        --out results/slgnn_v2/evaluations/case07
"""
...
def main():
    ...
    out_dir = (Path(args.out) if args.out else
               ROOT / "results" / "slgnn_v2" / "evaluations" / f"{ck_path.stem}_{case_name}")
