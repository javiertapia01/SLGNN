...
def test_delta_L_supervises_the_tangential_dissipation_head():
    """La nueva salida rotacional debe ser aprendible, no sólo no nula."""
    ...
    assert float(result.delta_L.detach().norm()) > 1e-8
    result.delta_L.pow(2).sum().backward()
    grad = sum(
        float(p.grad.abs().sum())
        for name, p in model.named_parameters()
        if name.startswith("head_Psi.tangential_coef_net") and p.grad is not None
    )
    assert grad > 0.0
